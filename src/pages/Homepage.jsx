...
export const Homepage = ({ categories }) => {
    ...
    const fetchCategoriesProducts = async (tempCat) => {
        try {
            const response = await apiClient.get(`${authToken ? "/categoryproducts" : "categoryguestproducts"}`);
            setFeatureCat(response.data.data)
            response.data.data.map((item, index) => { index < 5 ? tempCat.push(item.category_name) : console.log("") })
    ...
    const fetchBrandProducts = async (tempBrand) => {
        ...
        try {
            const response = await apiClient.get(`${authToken ? "/brandproducts" : "/brandguestproducts"}`);
            setBrandCat(response.data.data)
            response.data.data.map((item, index) => { index < 5 ? tempBrand.push(item.brand_name) : console.log("") })
    ...
    return (
        <div>
            ...
            <FeatureProduct featureCat={featureCat} featureCatList={featureCatList} selectedCat={selectedCat} featureCatLoader={featureCatLoader} setSelectedCat={setSelectedCat} />
            <FeatureBrand brandCat={brandCat} brandCatList={brandCatList} selectedBrand={selectedBrand} brandCatLoader={brandCatLoader} setSelectedBrand={setSelectedBrand} />
            <FeatureClearance />
            <TimerBanner />
            <BlogsCard data={data} />
        </div>
