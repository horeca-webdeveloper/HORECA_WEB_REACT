...
export const FeatureProduct = ({ featureCat, featureCatList, selectedCat, featureCatLoader, setSelectedCat }) => {
            ...
            <div className="grid grid-cols-5 gap-5">
                {featureCatLoader ? (
                    ...
                    {products && products.length > 0 ? products.map((product, index) => (
                        index < 10 ? <ProductCard key={index} classes="col-span-1 mt-1" product={product} /> : null
                    )) : <p className="col-span-5 font-semibold text-center text-base">No Product Found</p>}
                </React.Fragment>}
            </div>
        </Wrapper>
    );
